...
import { Users, UserCheck, TrendingUp, DollarSign, Target, Heart } from 'lucide-react';
...
const processDemographicData = (campaigns: Campaign[]) => {
  ...
  const allDemographics: (DemographicBreakdown & { spend: number, revenue: number })[] = [];
  ...
  Object.values(finalAggregate).forEach(data => {
    ...
    ageGroupRevenueMap[age] = (ageGroupRevenueMap[age] || 0) + data.total_revenue;

    // 3. Prepare Table Data (for Table)
    const ctr = data.impressions > 0 ? (data.clicks / data.impressions) * 100 : 0;
    const conversion_rate = data.clicks > 0 ? (data.conversions / data.clicks) * 100 : 0;
    const roas = data.total_spend > 0 ? data.total_revenue / data.total_spend : 0;
  ...
  const totalSpendByAge = ageGroups.map(group => ({
    label: group,
    value: ageGroupSpendMap[group],
    color: '#F59E0B'
  })).filter(d => d.value > 0);

  const totalRevenueByAge = ageGroups.map(group => ({
    label: group,
  ...
  return { stats, tableData: sortedTableData, totalSpendByAge, totalRevenueByAge };
};
...
export default function DemographicView() {
  ...
  const { stats, tableData, totalSpendByAge, totalRevenueByAge } = useMemo(() => {
            ...
            <>
              ...
              <div className="mb-6 sm:mb-8">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-6 gap-3 sm:gap-4">
                  ...
                  <CardMetric
                    title="Male Spend"
                    value={`$${stats.totalSpendMale?.toFixed(0).toLocaleString() || 0}`}
                    icon={<DollarSign className="h-5 w-5" />}
                  />
                  <CardMetric
                    title="Male Revenue"
                    value={`$${stats.totalRevenueMale?.toFixed(0).toLocaleString() || 0}`}
                  ...
                  <CardMetric
                    title="Female Spend"
                    value={`$${stats.totalSpendFemale?.toFixed(0).toLocaleString() || 0}`}
                    icon={<DollarSign className="h-5 w-5" />}
                  />
                  <CardMetric
                    title="Female Revenue"
                    value={`$${stats.totalRevenueFemale?.toFixed(0).toLocaleString() || 0}`}
                    icon={<TrendingUp className="h-5 w-5" />}
                    className="text-green-400"
                  />
                </div>
              ...
              <div className="overflow-x-auto w-full max-w-full">
                <Table
                  ...
                  maxHeight="500px"
                  columns={[
                    { key: 'gender', header: 'Gender', width: '10%', sortable: true, sortType: 'string',
                      render: (value) => (
                        <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${
                          value === 'Male' ? 'bg-blue-900 text-blue-300' : 'bg-pink-900 text-pink-300'
                        }`}>
                          {value}
                        </span>
                      )
                    },
                    { key: 'age_group', header: 'Age Group', width: '10%', align: 'center', sortable: true, sortType: 'string' },
                    { key: 'impressions', header: 'Impressions', width: '12%', align: 'right', sortable: true, sortType: 'number', render: (value) => value.toLocaleString() },
                    { key: 'clicks', header: 'Clicks', width: '12%', align: 'right', sortable: true, sortType: 'number', render: (value) => value.toLocaleString() },
                    { key: 'conversions', header: 'Conversions', width: '12%', align: 'right', sortable: true, sortType: 'number', render: (value) => value.toLocaleString() },
                    { key: 'ctr', header: 'CTR', width: '10%', align: 'right', sortable: true, sortType: 'number', render: (value) => `${value}%` },
                    { key: 'conversion_rate', header: 'Conv. Rate', width: '12%', align: 'right', sortable: true, sortType: 'number', render: (value) => `${value}%` },
                    { 
                        key: 'roas', 
                        header: 'ROAS', 
                        width: '9%', 
