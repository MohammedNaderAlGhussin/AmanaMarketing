...
export default function RegionView() {
  ...
  const regionalData = useMemo(() => {
    if (!marketingData?.campaigns) return { revenue: [], spend: [], combined: [] };

    const regionalRevenue: { [key: string]: number } = {};
    const regionalSpend: { [key: string]: number } = {};
    const regionalCombined: { [key: string]: { revenue: number; spend: number; country: string } } = {};
    ...
    marketingData.campaigns.forEach(campaign => {
      campaign.regional_performance?.forEach(region => {
        ...
        if (!regionalCombined[regionKey]) {
          regionalCombined[regionKey] = { revenue: 0, spend: 0, country: region.country };
        }
    ...
    const revenueData = Object.entries(regionalRevenue).map(([region, revenue]) => ({
      region,
    ...
    const combinedData = Object.entries(regionalCombined).map(([region, data]) => ({
      region,
  ...
  const regionalMetrics = useMemo(() => {
    ...
    const totalRevenue = regionalData.combined.reduce((sum, region) => sum + region.revenue, 0);
    const totalSpend = regionalData.combined.reduce((sum, region) => sum + region.spend, 0);
    const averageROAS = totalSpend > 0 ? totalRevenue / totalSpend : 0;
    const regionCount = regionalData.combined.length;

    // Find top performing region
    const topRegion = regionalData.combined.reduce((top, region) => 
      region.revenue > top.revenue ? region : top, regionalData.combined[0]
    );
          ...
          {marketingData && regionalMetrics && (
            <>
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-6 sm:mb-8">
                ...
                <CardMetric
                  title="Total Revenue"
                  value={`$${Math.round(regionalMetrics.totalRevenue).toLocaleString()}`}
                ...
                <CardMetric
                  title="Total Spend"
                  value={`$${Math.round(regionalMetrics.totalSpend).toLocaleString()}`}
              ...
              <div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <h4 className="text-md font-semibold text-green-400 mb-3">Revenue Leaders</h4>
                  ...
                  <div>
                    <h4 className="text-md font-semibold text-blue-400 mb-3">Spend Leaders</h4>
                    <div className="space-y-2">
                      {regionalData.spend
                        .sort((a, b) => b.spend - a.spend)
                        .slice(0, 3)
